...
const endpointInfo = new Schema({
  ...
  headers: { type: Object },
  authorizationContent: { type: String, required: true },
  authorizationMethod: { type: String, required: true },
...
const configInfo = new Schema({
  userId: { type: String, required: true },
  ticketRequest: { type: endpointInfo, required: true },
  paymentStatusRequest: { type: endpointInfo, required: true },
});
module.exports = {
  Payments: mongoose.model("payments", Payments, "payments"),
  ApprovedPaymentInfo: mongoose.model(
    "ApprovedPaymentInfo",
    ApprovedPaymentInfo,
    "ApprovedPaymentInfo"
  ),
  paymentsOurClients: mongoose.model(
    "paymentsOurClients",
    paymentsOurClients,
    "paymentsOurClients"
  ),

  ConfigInfo: mongoose.model("ConfigInfo", configInfo, "ConfigInfo"),
};
