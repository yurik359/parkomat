...
const AddParkomat = ({ closeModal, setCloseModal, addOneMoreParkomat }) => {
  ...
  const { typeOfmodal, paymentsInfo } = useSelector(
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (
      formValues.paymentValue.namePayment === "" ||
      formValues.paymentValue.namePayment === "..."
    )
      return setIsNamePayment(true);
    try {
  ...
  const handleEditParkomat = async (e) => {
    ...
    try {
      ...
      if (res && res.data) {
        dispatch(
          updateParkomat({ indexOfParkomat, updatedParkomat: res.data })
        );
      }
  ...
  const handleGetCoordinate = async (e) => {
    try {
      ...
      if (res && res.status == "200") {
        ...
        const street =
          $(".street-address").text().length > 1
            ? $(".street-address").text() + ","
            : "";
        const locality =
          $(".locality").text().length > 1 ? $(".locality").text() + "," : "";
        const region =
          $(".region").text().length > 1 ? $(".region").text() + "," : "";

        const lat = res.data.result.geometry.location.lat;
        const lon = res.data.result.geometry.location.lng;

        dispatch(
          changeLocationValue(`${street} ${locality} ${region} ${lat}, ${lon}`)
        );
  ...
  useEffect(() => {
    ...
    if (paymentsInfo && paymentsInfo.paymentSystem&&paymentsInfo.paymentSystem.length >= 1) {
      setIsFondy(
        paymentsInfo.paymentSystem.some(
          (e) => e.paymentSystem === "Fondy" && e.paymentApiKey && e.secretKey
        )
      );
      setIsStripe(
        paymentsInfo.paymentSystem.some(
          (e) => e.paymentSystem === "Stripe" && e.paymentApiKey && e.secretKey
        )
      );
    }
  }, [paymentsInfo]);
    ...
    >
      ...
      >
        ...
        <div className="add-parkoma__location-section">
          ...
          {!closeModal && (
            <div className="map-container" style={{ width: "90%" }}>
        ...
        >
          ...
          <option value="fondy" style={{ display: isFondy ? "block" : "none" }}>
            Fondy
          </option>
          <option
            value="Stripe"
            style={{ display: isStripe ? "block" : "none" }}
          >
            Stripe
          </option>
        </select>
        ...
        >
          <input
            checked={formValues.isSupportedByCarNumber}
            onChange={(e) =>
              dispatch(changeIsSupportedByCarNumber(e.target.checked))
            }
            type="checkbox"
            style={{ margin: "0 10px -0.5px 0", height: 20, width: 15 }}
          />
        ...
        <div
          ...
          style={{
            display: deleteIcon ? "block" : "none",
            display: formValues.picValue ? "block" : "none",
          }}
        >
          <img
